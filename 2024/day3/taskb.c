...
typedef enum { ENABLED, DISABLED } Mode;
typedef struct {
    int first_digit;
    int second_digit;
} MulDigits;
...
Mode mode = ENABLED;
int check_mode_switch(char *str) {
    if (str[0] == 'd' && str[1] == 'o' && str[2] == ascii_leftpar &&
        str[3] == ascii_rightpar) {
        mode = ENABLED;
    }
    if (str[0] == 'd' && str[1] == 'o' && str[2] == 'n' &&
        str[3] == ascii_single_quote && str[4] == 't' &&
        str[5] == ascii_leftpar && str[6] == ascii_rightpar) {
        mode = DISABLED;
    }
}
...
int check_buf_for_pattern(char *str, int size, MulDigits *digits) {
    // printf("str: %s\n", str);
    check_mode_switch(str);
    /*
    if (mode == ENABLED) {
        printf("Enable, str:%s\n", str);
    } else
        printf("*Disabled, str:%s\n", str);
    */

    if (str[0] == ascii_m && str[1] == ascii_u && str[2] == ascii_l &&
        str[3] == ascii_leftpar && mode == ENABLED) {
